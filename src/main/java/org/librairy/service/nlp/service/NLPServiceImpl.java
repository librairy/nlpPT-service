...
public class NLPServiceImpl implements org.librairy.service.nlp.facade.model.NlpService {
    ...
    public String tokens(String text, List<PoS> filter, Form form, boolean multigrams) throws AvroRemoteException {
        List<Annotation> annotations = serviceManager.getLemProService(Thread.currentThread()).annotations(text, filter);
        if (multigrams){
    ...
    public List<Annotation> annotations(String text, List<PoS> filter, boolean multigrams, boolean references) throws AvroRemoteException {
        List<Annotation> annotations = addExtra(serviceManager.getLemProService(Thread.currentThread()).annotations(text, filter), text, multigrams, references);
    ...
    public List<Group> groups(String text, List<PoS> filter, boolean multigrams, boolean references) throws AvroRemoteException {
        List<Annotation> annotations = addExtra(serviceManager.getLemProService(Thread.currentThread()).annotations(text, filter), text, multigrams, references).stream().filter(a -> CharMatcher.javaLetterOrDigit().matchesAnyOf(a.getToken().getLemma())).collect(Collectors.toList());
